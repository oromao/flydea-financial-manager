    ...
    <div className={cn("flex flex-wrap gap-2 items-center", className)} role="group" aria-label="Filtros">
      ...
      {active.length > 0 && onClear && (
        <button
          type="button"
          onClick={onClear}
          className="text-xs font-bold uppercase tracking-wider text-on-surface-variant/60 hover:text-on-surface-variant transition-colors px-3 py-1.5 rounded-lg hover:bg-accent/5"
        >
          Limpar filtros
        </button>
      )}
